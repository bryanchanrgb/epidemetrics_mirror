...
'''
cur.execute("SELECT properties FROM country_statistics WHERE source = 'EVS';")
test = pd.DataFrame(cur.fetchall(), columns=[desc[0] for desc in cur.description])
...
evs_cols = {}
for i in eval(country_statistics_evs.loc[215,"properties"]):
    evs_cols[i] = eval(country_statistics_evs.loc[215,"properties"])[i]["Label"]

# create dataframe to store data dictionary for each question and options
evs_labels = pd.DataFrame()
temp_dict = eval(country_statistics_evs.loc[215,"properties"])
...
for k in tqdm(country_statistics_evs.index):
    temp_dict = eval(country_statistics_evs.loc[k,"properties"])
...
# same problem for WVS, every country has the same values
