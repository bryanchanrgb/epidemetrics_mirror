...
class PreAlgo:
    ...
    def init_country(self, country: str, field: str = 'new_per_day_smooth') -> (DataFrame, DataFrame):
    ...
    def run(self, country: str, field: str = 'new_per_day_smooth', plot: bool = False) -> (
        ...
        data, pre_algo = self.init_country(country, field=field)
        ...
        return data, pre_algo, spikes_removed, prominence_updater

    def plot(self, data: DataFrame, results: DataFrame, field: str):
        plt.plot(data[field].values)
        plt.scatter(results['location'].values, data[field].values[results['location'].values.astype(int)],
                    color='red', marker='o')
        plt.show()
