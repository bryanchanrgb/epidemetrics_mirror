...
class Epidemetrics:
    def __init__(self, data_provider: DataProvider):
        ...
        self.epidemiology = data_provider.epidemiology
        self.testing = data_provider.testing
        self.wbi_table = data_provider.wbi_table
        self.epidemiology_series = data_provider.epidemiology_series
        self.gsi_table = data_provider.gsi_table
    ...
    def prepare_output_dirs(self):
        try:
            ...
            print("Error: %s : %s" % (self.plot_path, e.strerror))
        Path(os.path.dirname(self.plot_path)).mkdir(parents=True, exist_ok=True)
    ...
    def _sub_algorithm_c(self, sub_a, sub_b, country, field='new_per_day_smooth', plot=False):
        ...
        result_peaks_d = result_peaks_c[
            (result_peaks_c['prominence'] >= self.prominence_height_threshold * result_peaks_c['y_position'])]
        result_troughs_d = result_troughs_c[
            list((result_peaks_c['prominence'] >= self.prominence_height_threshold * result_peaks_c['y_position']))]
        result_troughs_d = result_troughs_d[~pd.isnull(result_troughs_d['peak_ind'])]
        results = pd.concat([result_peaks_d, result_troughs_d], ignore_index=True).sort_values(
            by='location').reset_index(drop=True)
        ...
        for i, death_peak in enumerate(deaths_sub_c['location']):
            ...
            # if peak in cases_sub_b output use the most prominent one
                ...
                candidates = cases_sub_b[(cases_sub_b['peak_ind'] == 1) &
                                         (cases_sub_b['location'] >= death_peak - self.d_match) & (
    ...
    def epi_find_peaks(self, country, plot=False, save=False):
    ...
    def _classify(self, country, field='new_per_day_smooth'):
        ...
        if field == 'new_per_day_smooth':
            ...
            if len(deaths) < 3:
                return 0, None
            genuine_peaks = self.epi_find_peaks(country, plot=False, save=False)
    ...
    def _get_epi_panel(self):
        ...
        for country in tqdm(np.sort(self.epidemiology_series['countrycode'].unique()),
            ...
            data['population_density'] = np.nan if len(
                self.wbi_table[self.wbi_table['countrycode'] == country]) == 0 else \
            ...
            data['t0_relative'] = np.nan if len(
                country_series[((country_series['confirmed'] /
                                 data['population']) * self.rel_to_constant >= self.rel_t0_threshold)][
                    'date']) == 0 else \
                country_series[((country_series['confirmed'] /
                                 data['population']) * self.rel_to_constant >= self.rel_t0_threshold)]['date'].iloc[
                    0]
    ...
    def _mann_whitney(self, data, field='gni_per_capita'):
        ...
        return pd.mwu(x, y, tail='one-sided')
